...
async def get_current_user(
    ...
    if not token:
        raise HTTPException(
            ...
            headers={"WWW-Authenticate": 'Bearer realm="lablet-controller"'},
        )
...
async def require_internal_api_key(
    ...
    """Require valid internal API key for service-to-service calls.

    ADR-017: Used for BFF endpoints called by control-plane-api.
    ...
    Raises:
        HTTPException: 401 if API key missing, 403 if invalid
    """
    ...
    if not x_api_key:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Internal API key required. Provide X-API-Key header.",
        )
