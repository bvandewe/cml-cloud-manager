...
def parse_event_timestamp(timestamp_str: str) -> datetime:
    ...
    raise ValueError(f"Timestamp '{timestamp_str}' does not match any expected format. Expected ISO 8601 format like '2025-11-19T10:25:32' or '2025-11-19T10:25:32Z'")
...
def filter_relevant_events(
    ...
    log.debug(f"Filtered {len(filtered)} relevant events from {len(events)} total ({excluded_count} excluded by filters)")
