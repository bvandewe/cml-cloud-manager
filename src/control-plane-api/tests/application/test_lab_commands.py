...
def _make_discovered_lab(
    lab_record_id: str = "lr-001", lab_id: str = "lab-001", worker_id: str = "worker-001", status: LabRecordStatus = LabRecordStatus.DISCOVERED, pending_action: str | None = None
) -> LabRecord:
    """Create a LabRecord via the discover factory and optionally force state."""
    lab = LabRecord.discover(lab_id=lab_id, worker_id=worker_id, title="Test Lab", description="A test lab for commands", state="DEFINED_ON_CORE", owner_username="admin", node_count=3, link_count=2)
...
def _make_mock_session(session_id: str = "lablet-001", lab_record_id: str | None = None) -> MagicMock:
...
class TestStartLabRecordCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> StartLabRecordCommandHandler:
        return StartLabRecordCommandHandler(lab_record_repository=mock_lab_repository)
...
class TestStopLabRecordCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> StopLabRecordCommandHandler:
        return StopLabRecordCommandHandler(lab_record_repository=mock_lab_repository)
...
class TestWipeLabRecordCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> WipeLabRecordCommandHandler:
        return WipeLabRecordCommandHandler(lab_record_repository=mock_lab_repository)
...
class TestDeleteLabRecordCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> DeleteLabRecordCommandHandler:
        return DeleteLabRecordCommandHandler(lab_record_repository=mock_lab_repository)
...
class TestCloneLabRecordCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> CloneLabRecordCommandHandler:
        return CloneLabRecordCommandHandler(lab_record_repository=mock_lab_repository)
    ...
    async def test_clone_success(self, handler: CloneLabRecordCommandHandler, mock_lab_repository: MagicMock) -> None:
        ...
        result = await handler.handle_async(CloneLabRecordCommand(source_lab_record_id="lr-001", title="My Clone", cloned_by="user:tester"))
...
class TestArchiveLabRecordCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> ArchiveLabRecordCommandHandler:
        return ArchiveLabRecordCommandHandler(lab_record_repository=mock_lab_repository)
...
class TestBindLabToLabletCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> BindLabToLabletCommandHandler:
        return BindLabToLabletCommandHandler(lab_record_repository=mock_lab_repository, lablet_session_repository=mock_session_repository)

    @pytest.mark.asyncio
    async def test_bind_success(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
        ...
        result = await handler.handle_async(BindLabToLabletCommand(lab_record_id="lr-001", lablet_session_id="lablet-001", role="primary"))
    ...
    @pytest.mark.asyncio
    async def test_bind_lab_not_found(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_bind_terminal_lab(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_bind_session_not_found(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_bind_duplicate_active_binding(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_bind_session_already_bound_to_other_lab(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_bind_invalid_role(self, handler: BindLabToLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
        ...
        result = await handler.handle_async(BindLabToLabletCommand(lab_record_id="lr-001", lablet_session_id="lablet-001", role="INVALID_ROLE"))
...
class TestUnbindLabFromLabletCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> UnbindLabFromLabletCommandHandler:
        return UnbindLabFromLabletCommandHandler(lab_record_repository=mock_lab_repository, lablet_session_repository=mock_session_repository)

    @pytest.mark.asyncio
    async def test_unbind_success(self, handler: UnbindLabFromLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
        ...
        result = await handler.handle_async(UnbindLabFromLabletCommand(lab_record_id="lr-001", lablet_session_id="lablet-001", reason="user_request"))
    ...
    @pytest.mark.asyncio
    async def test_unbind_lab_not_found(self, handler: UnbindLabFromLabletCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_unbind_session_not_found(self, handler: UnbindLabFromLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_unbind_no_active_binding(self, handler: UnbindLabFromLabletCommandHandler, mock_lab_repository: MagicMock, mock_session_repository: MagicMock) -> None:
...
class TestUpdateLabRecordStatusCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> UpdateLabRecordStatusCommandHandler:
        return UpdateLabRecordStatusCommandHandler(lab_record_repository=mock_lab_repository)

    @pytest.mark.asyncio
    async def test_update_status_via_typed_value(self, handler: UpdateLabRecordStatusCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_status_already_at_target(self, handler: UpdateLabRecordStatusCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_status_not_found(self, handler: UpdateLabRecordStatusCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_status_no_status_provided(self, handler: UpdateLabRecordStatusCommandHandler, mock_lab_repository: MagicMock) -> None:
...
class TestUpdateLabTopologyCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> UpdateLabTopologyCommandHandler:
        return UpdateLabTopologyCommandHandler(lab_record_repository=mock_lab_repository)

    @pytest.mark.asyncio
    async def test_update_topology_success(self, handler: UpdateLabTopologyCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_topology_not_found(self, handler: UpdateLabTopologyCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_update_topology_terminal_state(self, handler: UpdateLabTopologyCommandHandler, mock_lab_repository: MagicMock) -> None:
...
class TestRecordLabRunCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> RecordLabRunCommandHandler:
        return RecordLabRunCommandHandler(lab_record_repository=mock_lab_repository)

    @pytest.mark.asyncio
    async def test_record_run_success(self, handler: RecordLabRunCommandHandler, mock_lab_repository: MagicMock) -> None:
        ...
        result = await handler.handle_async(
            RecordLabRunCommand(
                lab_record_id="lr-001", started_at="2025-01-01T10:00:00+00:00", stopped_at="2025-01-01T11:30:00+00:00", started_by="user:admin", stop_reason="timeslot_end", final_state="stopped"
            )
        )
    ...
    @pytest.mark.asyncio
    async def test_record_run_not_found(self, handler: RecordLabRunCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_record_run_minimal_fields(self, handler: RecordLabRunCommandHandler, mock_lab_repository: MagicMock) -> None:
...
class TestCompleteLabActionCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> CompleteLabActionCommandHandler:
        return CompleteLabActionCommandHandler(lab_record_repository=mock_lab_repository)

    @pytest.mark.asyncio
    async def test_complete_start_action(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_complete_stop_action(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_complete_no_action_and_no_pending(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_complete_not_found(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_start_action_opens_run(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_stop_action_closes_run(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
        """Completing a 'stop' action should close the open run in run_history_v2."""
        lab = _make_discovered_lab(status=LabRecordStatus.STOPPING, pending_action="stop")
        # Simulate an open run from a previous start
        open_run = LabRunRecord(run_id="run-existing", started_at=datetime.now(timezone.utc), stopped_at=None, duration_seconds=None, started_by="user")
    ...
    @pytest.mark.asyncio
    async def test_wipe_action_closes_run(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
        ...
        open_run = LabRunRecord(run_id="run-wipe", started_at=datetime.now(timezone.utc), stopped_at=None, duration_seconds=None, started_by="user")
        lab.state.run_history_v2.append(open_run.to_dict())
        mock_lab_repository.get_by_id_async = self.create_async_mock(return_value=lab)
    ...
    @pytest.mark.asyncio
    async def test_stop_no_open_run_graceful(self, handler: CompleteLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
...
class TestFailLabActionCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> FailLabActionCommandHandler:
        return FailLabActionCommandHandler(lab_record_repository=mock_lab_repository)

    @pytest.mark.asyncio
    async def test_fail_action_success(self, handler: FailLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
        """Fail a pending action records the error."""
        lab = _make_discovered_lab(pending_action="start")
        mock_lab_repository.get_by_id_async = self.create_async_mock(return_value=lab)

        result = await handler.handle_async(FailLabActionCommand(lab_record_id="lr-001", error_message="CML API returned 500", transition_to_error=False))
    ...
    @pytest.mark.asyncio
    async def test_fail_action_transition_to_error(self, handler: FailLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
        """Fail a pending action with transition_to_error marks lab as ERROR."""
        lab = _make_discovered_lab(pending_action="wipe")
        mock_lab_repository.get_by_id_async = self.create_async_mock(return_value=lab)

        result = await handler.handle_async(FailLabActionCommand(lab_record_id="lr-001", error_message="Wipe failed", transition_to_error=True))

        assert result.is_success
        assert result.status_code == 200

    @pytest.mark.asyncio
    async def test_fail_action_no_pending(self, handler: FailLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fail_action_not_found(self, handler: FailLabActionCommandHandler, mock_lab_repository: MagicMock) -> None:
...
class TestDiscoverLabRecordsCommand(BaseTestCase):
    ...
    @pytest.fixture
    def handler(self, mock_lab_repository: MagicMock) -> DiscoverLabRecordsCommandHandler:
        return DiscoverLabRecordsCommandHandler(lab_record_repository=mock_lab_repository)

    @pytest.mark.asyncio
    async def test_discover_new_labs(self, handler: DiscoverLabRecordsCommandHandler, mock_lab_repository: MagicMock) -> None:
        ...
        result = await handler.handle_async(DiscoverLabRecordsCommand(worker_id="worker-001", labs=labs_data, source="test"))
    ...
    @pytest.mark.asyncio
    async def test_discover_updates_existing_labs(self, handler: DiscoverLabRecordsCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_discover_orphans_missing_labs(self, handler: DiscoverLabRecordsCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_discover_missing_worker_id(self, handler: DiscoverLabRecordsCommandHandler) -> None:
    ...
    @pytest.mark.asyncio
    async def test_discover_handles_lab_missing_id(self, handler: DiscoverLabRecordsCommandHandler, mock_lab_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_discover_mixed_new_and_existing(self, handler: DiscoverLabRecordsCommandHandler, mock_lab_repository: MagicMock) -> None:
