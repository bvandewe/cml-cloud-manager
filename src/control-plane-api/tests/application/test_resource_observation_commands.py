...
import pytest
from application.commands.lablet_session.record_resource_observation_command import RecordResourceObservationCommand, RecordResourceObservationCommandHandler
from application.commands.lablet_session.request_resource_observation_command import RequestResourceObservationCommand, RequestResourceObservationCommandHandler
...
class TestRecordResourceObservationCommandHandler:
    ...
    def _make_handler(self, mock_session_repository: MagicMock) -> RecordResourceObservationCommandHandler:
        return RecordResourceObservationCommandHandler(lablet_session_repository=mock_session_repository)

    @pytest.mark.asyncio
    async def test_running_session_ok(self, mock_session_repository: MagicMock) -> None:
        ...
        command = RecordResourceObservationCommand(session_id="session-001", observed_resources=SAMPLE_RESOURCES, observed_ports=SAMPLE_PORTS)
        result = await handler.handle_async(command)

        assert result.is_success
        assert result.status_code == 200
        session.record_resource_observation.assert_called_once_with(observed_resources=SAMPLE_RESOURCES, observed_ports=SAMPLE_PORTS)
        mock_session_repository.update_async.assert_called_once_with(session)

    @pytest.mark.asyncio
    async def test_collecting_session_ok(self, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_wrong_state_fails(self, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_not_found_fails(self, mock_session_repository: MagicMock) -> None:
        """Recording observations for non-existent session returns 404."""
        mock_session_repository.get_by_id_async.return_value = None

        handler = self._make_handler(mock_session_repository)

        command = RecordResourceObservationCommand(session_id="nonexistent", observed_resources=SAMPLE_RESOURCES, observed_ports=SAMPLE_PORTS)
        result = await handler.handle_async(command)
...
class TestRequestResourceObservationCommandHandler:
    ...
    def _make_handler(self, mock_session_repository: MagicMock) -> RequestResourceObservationCommandHandler:
        return RequestResourceObservationCommandHandler(lablet_session_repository=mock_session_repository)
    ...
    @pytest.mark.asyncio
    async def test_no_cml_lab_fails(self, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_no_worker_fails(self, mock_session_repository: MagicMock) -> None:
