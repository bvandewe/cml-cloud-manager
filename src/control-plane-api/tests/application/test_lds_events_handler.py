...
import pytest
from application.events.integration.lds_events import LdsSessionEndedIntegrationEventV1, LdsSessionPausedIntegrationEventV1, LdsSessionRunningIntegrationEventV1
from application.events.integration.lds_events_handler import LdsSessionEndedHandler, LdsSessionPausedHandler, LdsSessionRunningHandler
...
def _make_session(session_id: str = "session-001", status: LabletSessionStatus = LabletSessionStatus.READY) -> MagicMock:
...
def _make_running_handler(mock_session_repository, mock_deduplication, mock_settings) -> LdsSessionRunningHandler:
...
class TestLdsSessionRunningHandler:
    ...
    @pytest.mark.asyncio
    async def test_happy_path_ready_to_running(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        handler = _make_running_handler(mock_session_repository, mock_deduplication, mock_settings)

        event = LdsSessionRunningIntegrationEventV1(aggregate_id="sess-123", session_id="sess-123")
    ...
    @pytest.mark.asyncio
    async def test_session_not_found(self, mock_session_repository, mock_deduplication, mock_settings):
        """Handler logs error when session not found."""
        mock_session_repository.get_by_id_async = AsyncMock(return_value=None)

        handler = _make_running_handler(mock_session_repository, mock_deduplication, mock_settings)

        event = LdsSessionRunningIntegrationEventV1(aggregate_id="nonexistent-id", session_id="nonexistent-id")
    ...
    @pytest.mark.asyncio
    async def test_session_not_in_ready_state(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        event = LdsSessionRunningIntegrationEventV1(aggregate_id="sess-456", session_id="sess-456")
    ...
    @pytest.mark.asyncio
    async def test_deduplication_skips_already_processed(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        event = LdsSessionRunningIntegrationEventV1(aggregate_id="sess-789", session_id="sess-789")
    ...
    @pytest.mark.asyncio
    async def test_disabled_via_settings(self, mock_session_repository, mock_deduplication, mock_settings):
        """Handler skips processing when LDS CloudEvents are disabled."""
        mock_settings.lds_cloudevent_enabled = False

        handler = _make_running_handler(mock_session_repository, mock_deduplication, mock_settings)

        event = LdsSessionRunningIntegrationEventV1(aggregate_id="sess-disabled", session_id="sess-disabled")
    ...
    @pytest.mark.asyncio
    async def test_invalid_state_transition_error(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        session.mark_running.side_effect = InvalidStateTransitionError(from_state=LabletSessionStatus.READY, to_state=LabletSessionStatus.RUNNING, message="Cannot transition from READY to RUNNING")
        mock_session_repository.get_by_id_async = AsyncMock(return_value=session)

        handler = _make_running_handler(mock_session_repository, mock_deduplication, mock_settings)

        event = LdsSessionRunningIntegrationEventV1(aggregate_id="sess-err", session_id="sess-err")
    ...
    @pytest.mark.asyncio
    async def test_resolves_session_id_from_session_id_field(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        event = LdsSessionRunningIntegrationEventV1(aggregate_id="", session_id="sess-fallback")
...
class TestLdsSessionPausedHandler:
    ...
    @pytest.mark.asyncio
    async def test_paused_handler_logs_without_error(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        event = LdsSessionPausedIntegrationEventV1(aggregate_id="sess-paused", session_id="sess-paused", reason="idle_timeout")
...
class TestLdsSessionEndedHandler:
    ...
    @pytest.mark.asyncio
    async def test_ended_handler_logs_without_error(self, mock_session_repository, mock_deduplication, mock_settings):
        ...
        event = LdsSessionEndedIntegrationEventV1(aggregate_id="sess-ended", session_id="sess-ended", reason="timeout", ended_by="system")
