...
import pytest
from application.commands.lablet_session.create_lablet_session_command import CreateLabletSessionCommand, CreateLabletSessionCommandHandler
from application.commands.lablet_session.schedule_lablet_session_command import ScheduleLabletSessionCommand, ScheduleLabletSessionCommandHandler
...
from application.commands.lablet_session.transition_lablet_session_command import TransitionLabletSessionCommand, TransitionLabletSessionCommandHandler
...
def _make_worker(worker_id: str = "worker-001", status: CMLWorkerStatus = CMLWorkerStatus.RUNNING) -> MagicMock:
...
class TestCreateLabletSessionCommandHandler:
    ...
    def _make_handler(self, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> CreateLabletSessionCommandHandler:
        return CreateLabletSessionCommandHandler(lablet_session_repository=mock_session_repository, lablet_definition_repository=mock_definition_repository)

    @pytest.mark.asyncio
    async def test_creates_session_with_valid_inputs(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        ...
        handler = self._make_handler(mock_session_repository, mock_definition_repository)

        now = datetime.now(timezone.utc)
        command = CreateLabletSessionCommand(definition_id="def-001", owner_id="user-001", timeslot_start=(now + timedelta(hours=1)).isoformat(), timeslot_end=(now + timedelta(hours=2)).isoformat())
    ...
    @pytest.mark.asyncio
    async def test_rejects_empty_definition_id(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        """Verify bad_request when definition_id is empty."""
        handler = self._make_handler(mock_session_repository, mock_definition_repository)

        command = CreateLabletSessionCommand(
            definition_id="", owner_id="user-001", timeslot_start=datetime.now(timezone.utc).isoformat(), timeslot_end=(datetime.now(timezone.utc) + timedelta(hours=1)).isoformat()
        )
    ...
    @pytest.mark.asyncio
    async def test_rejects_nonexistent_definition(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        ...
        now = datetime.now(timezone.utc)
        command = CreateLabletSessionCommand(
            definition_id="nonexistent", owner_id="user-001", timeslot_start=(now + timedelta(hours=1)).isoformat(), timeslot_end=(now + timedelta(hours=2)).isoformat()
        )
    ...
    @pytest.mark.asyncio
    async def test_rejects_inactive_definition(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rejects_duplicate_reservation_id(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        ...
        now = datetime.now(timezone.utc)
        command = CreateLabletSessionCommand(
            definition_id="def-001", owner_id="user-001", timeslot_start=(now + timedelta(hours=1)).isoformat(), timeslot_end=(now + timedelta(hours=2)).isoformat(), reservation_id="duplicate-res-001"
        )
...
class TestScheduleLabletSessionCommandHandler:
    ...
    def _make_handler(
        self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock, mock_worker_repository: MagicMock
    ) -> ScheduleLabletSessionCommandHandler:
        return ScheduleLabletSessionCommandHandler(
            mediator=mock_mediator, lablet_session_repository=mock_session_repository, lablet_definition_repository=mock_definition_repository, cml_worker_repository=mock_worker_repository
        )
    ...
    async def test_schedules_pending_session_on_running_worker(
        ...
        handler = self._make_handler(mock_mediator, mock_session_repository, mock_definition_repository, mock_worker_repository)

        command = ScheduleLabletSessionCommand(session_id="session-001", worker_id="worker-001", allocated_ports={"serial_1": 5041, "vnc_1": 5044}, lab_record_id="lr-001")
    ...
    async def test_rejects_scheduling_non_pending_session(
        ...
        command = ScheduleLabletSessionCommand(session_id="session-001", worker_id="worker-001", allocated_ports={"serial_1": 5041}, lab_record_id="lr-001")
    ...
    @pytest.mark.asyncio
    async def test_rejects_stopped_worker(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock, mock_worker_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rejects_insufficient_capacity(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock, mock_worker_repository: MagicMock) -> None:
...
class TestTransitionLabletSessionCommandHandler:
    ...
    def _make_handler(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock | None = None) -> TransitionLabletSessionCommandHandler:
        if mock_definition_repository is None:
            mock_definition_repository = MagicMock(spec=LabletDefinitionRepository)
            mock_definition_repository.get_by_id_async = AsyncMock(return_value=None)
        return TransitionLabletSessionCommandHandler(mediator=mock_mediator, lablet_session_repository=mock_session_repository, lablet_definition_repository=mock_definition_repository)

    @pytest.mark.asyncio
    async def test_transitions_to_running(self, mock_mediator: MagicMock, mock_session_repository: MagicMock) -> None:
        """Verify transition to RUNNING succeeds."""
        session = _make_session(status=LabletSessionStatus.READY)
        mock_session_repository.get_by_id_async = AsyncMock(return_value=session)

        handler = self._make_handler(mock_mediator, mock_session_repository)
    ...
    @pytest.mark.asyncio
    async def test_transitions_to_collecting(self, mock_mediator: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rejects_invalid_target_status(self, mock_mediator: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rejects_grading_without_dedicated_endpoint(self, mock_mediator: MagicMock, mock_session_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_stopping_releases_capacity(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        """Verify STOPPING transition releases worker capacity using definition resources."""
        session = _make_session(status=LabletSessionStatus.COLLECTING, worker_id="worker-001", definition_id="def-001")
        mock_session_repository.get_by_id_async = AsyncMock(return_value=session)
        ...
        handler = self._make_handler(mock_mediator, mock_session_repository, mock_definition_repository)
...
class TestTerminateLabletSessionCommandHandler:
    ...
    def _make_handler(
        self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock, mock_lab_record_repository: MagicMock | None = None
    ...
    @pytest.mark.asyncio
    async def test_terminates_running_session(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        """Verify successful termination of a running session."""
        session = _make_session(status=LabletSessionStatus.RUNNING, worker_id="worker-001")
        mock_session_repository.get_by_id_async = AsyncMock(return_value=session)
        ...
        command = TerminateLabletSessionCommand(session_id="session-001", terminated_by="admin", reason="Manual termination")
    ...
    @pytest.mark.asyncio
    async def test_rejects_terminating_already_terminated(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_rejects_empty_session_id(self, mock_mediator: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
