...
import pytest
from application.commands.worker.recalculate_worker_capacity_command import RecalculateWorkerCapacityCommand, RecalculateWorkerCapacityCommandHandler
...
def _make_worker(worker_id: str = "worker-001", session_ids: list[str] | None = None, allocated_cpu: int = 24, allocated_memory: int = 48, allocated_storage: int = 240) -> MagicMock:
    ...
    state.allocated_capacity = WorkerCapacity(cpu_cores=allocated_cpu, memory_gb=allocated_memory, storage_gb=allocated_storage)
...
def _make_session(session_id: str, status: LabletSessionStatus = LabletSessionStatus.RUNNING, definition_id: str = "def-001") -> MagicMock:
...
def _make_definition(definition_id: str = "def-001", cpu_cores: int = 4, memory_gb: int = 8, storage_gb: int = 50, node_count: int | None = None) -> MagicMock:
...
class TestRecalculateWorkerCapacityCommandHandler:
    ...
    def _make_handler(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> RecalculateWorkerCapacityCommandHandler:
        return RecalculateWorkerCapacityCommandHandler(
            cml_worker_repository=mock_worker_repository, lablet_session_repository=mock_session_repository, lablet_definition_repository=mock_definition_repository
        )

    @pytest.mark.asyncio
    async def test_worker_not_found(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        """Returns 404 when worker does not exist."""
        mock_worker_repository.get_by_id_async.return_value = None

        handler = self._make_handler(mock_worker_repository, mock_session_repository, mock_definition_repository)
    ...
    @pytest.mark.asyncio
    async def test_no_sessions_resets_to_zero(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        ...
        worker.recalculate_capacity.assert_called_once_with(
            recalculated_cpu_cores=0, recalculated_memory_gb=0, recalculated_storage_gb=0, recalculated_max_nodes=None, active_session_ids=[], stale_session_ids=[]
        )

    @pytest.mark.asyncio
    async def test_all_sessions_stale_resets_to_zero(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        """All tracked sessions in terminal states → capacity reset to zero."""
        worker = _make_worker(session_ids=["s1", "s2", "s3"], allocated_cpu=12, allocated_memory=24, allocated_storage=150)
        mock_worker_repository.get_by_id_async.return_value = worker
        ...
        worker.recalculate_capacity.assert_called_once_with(
            recalculated_cpu_cores=0, recalculated_memory_gb=0, recalculated_storage_gb=0, recalculated_max_nodes=None, active_session_ids=[], stale_session_ids=["s1", "s2", "s3"]
        )

    @pytest.mark.asyncio
    async def test_mixed_active_and_stale_sessions(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        """Mixed active and stale sessions: only active sessions contribute capacity."""
        worker = _make_worker(session_ids=["active-1", "expired-1", "active-2"], allocated_cpu=24, allocated_memory=48, allocated_storage=240)
        mock_worker_repository.get_by_id_async.return_value = worker
        ...
        worker.recalculate_capacity.assert_called_once_with(
            recalculated_cpu_cores=6, recalculated_memory_gb=12, recalculated_storage_gb=80, recalculated_max_nodes=None, active_session_ids=["active-1", "active-2"], stale_session_ids=["expired-1"]
        )

    @pytest.mark.asyncio
    async def test_session_not_in_db_treated_as_stale(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_missing_definition_still_counts_session_active(self, mock_worker_repository: MagicMock, mock_session_repository: MagicMock, mock_definition_repository: MagicMock) -> None:
        ...
        worker.recalculate_capacity.assert_called_once_with(
            recalculated_cpu_cores=0, recalculated_memory_gb=0, recalculated_storage_gb=0, recalculated_max_nodes=None, active_session_ids=["s1"], stale_session_ids=[]
        )
