...
from integration.repositories.motor_pipeline_execution_repository import MongoPipelineExecutionRepository
...
from integration.services.cloud_event_publisher import CloudEventPublisher as LcmCloudEventPublisher
...
def create_app() -> FastAPI:
    ...
    # ADR-042: Standalone integration event publisher (replaces CommandHandlerBase.publish_cloud_event_async)
    builder.services.add_scoped(LcmCloudEventPublisher)
    ...
    def _register_child_repos(b: WebApplicationBuilder) -> None:
        ...
        def pipeline_execution_factory(sp: Any) -> MongoPipelineExecutionRepository:
            client = sp.get_required_service(AsyncIOMotorClient)
            serializer = sp.get_required_service(JsonSerializer)
